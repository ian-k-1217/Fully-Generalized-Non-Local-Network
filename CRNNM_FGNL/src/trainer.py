...
def train_model(nb_classes=20,
	...
	if train:
		...
		history = model.fit((X_train, M_train), Y_train, batch_size=batch_size,
							shuffle=True, epochs=nb_epochs,
							verbose=1, validation_data=((X_val, M_val), Y_val),
							callbacks=[F1_Metrics(valid_data=((X_val, M_val), Y_val)), checkpointer_loss, checkpointer_acc, checkpointer_f1, earlystopper])
	...
	y_score = model.predict((X_test, M_test))
	...
	if tsne:
		...
		intermed_tensor_func = tf.keras.backend.function([model.layers[0].input, model.layers[1].input],[model.layers[-3].output])
		X_rep = []
		
		# predict representation
		print("Predicting")
		for i in range(len(X_test)):
			X_rep.extend(intermed_tensor_func([np.expand_dims(X_test[i], axis = 0), np.expand_dims(M_test[i], axis = 0)])[0])
