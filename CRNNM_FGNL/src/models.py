...
def model(frequency_axis = 1, time_axis = 2, channel_axis = 3, class_num = 20, slice_length = 157):
	...
	mel_branch = tf.keras.layers.BatchNormalization(axis = frequency_axis)(mel_input);
	mel_branch_cb1 = conv_block(mel_branch, 64, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelBranch_Conv_1', pooling = False);
	mel_branch_cb2 = conv_block(mel_branch_cb1, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelBranch_Conv_2', pooling = True);
	mel_branch_cb3 = conv_block(mel_branch_cb2, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelBranch_Conv_3', pooling = True);
	mel_branch_cb4 = conv_block(mel_branch_cb3, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelBranch_Conv_4', pooling = True);
	
	
	## melody cnn branch
	# shape of input data (frequency, time, channels)
	melody_input = tf.keras.layers.Input(shape=(128, slice_length, 1));
	melody_branch = tf.keras.layers.BatchNormalization(axis = frequency_axis)(melody_input);
	melody_branch_cb1 = conv_block(melody_branch, 64, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelodyBranch_Conv_1', pooling = False);
	melody_branch_cb2 = conv_block(melody_branch_cb1, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelodyBranch_Conv_2', pooling = True);
	melody_branch_cb3 = conv_block(melody_branch_cb2, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelodyBranch_Conv_3', pooling = True);
	melody_branch_cb4 = conv_block(melody_branch_cb3, 128, kernel_size = (3, 3), activation = 'elu', pool_size = (2, 2), channel_axis = 3, block_name = 'MelodyBranch_Conv_4', pooling = True);
	
	
	####========================================================================================
	## concatenate 2 branches		
	cb4_concat = tf.keras.layers.concatenate([mel_branch_cb4, melody_branch_cb4], axis = -1, name = 'cb4_concat');
	cb3_concat = tf.keras.layers.concatenate([mel_branch_cb3, melody_branch_cb3], axis = -1, name = 'cb3_concat');
	
	x = FullyGeneralizedNonLocalLayer(cb4_concat, [cb4_concat, cb3_concat]);
	####========================================================================================
	## reshape
	...
	####========================================================================================
	model = tf.keras.Model(inputs = (mel_input, melody_input) , outputs = y);
